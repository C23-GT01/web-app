...
const EditInstagram = ({ data, refresh, closeModal, noClose }) => {
  const [contact, setContact] = useState({
    name: data.contact.instagram ? data.contact.instagram.name : "",
    url: data.contact.instagram ? data.contact.instagram.url : "",
  });
  ...
  const handleSubmit = async (event) => {
    ...
    if (contact.name === "" && contact.url === "") {
      const { instagram, ...restContact } = data.contact;
      contactData = {
        ...restContact,
      };
    } else {
        ...
        <form onSubmit={handleSubmit} className="grid gap-4">
          ...
          <div className="mb-4 md:col-span-2">
            ...
            <input
              ...
              className="w-full border rounded-md py-2 px-3"
              autoComplete="off"
              placeholder="https://facebook.co.id/example"
            />
